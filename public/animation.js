...
(function () {
  ...
  async function main() {
    ...
    // Loop de animación. Usamos requestAnimationFrame para mantener
    // 30fps; Puppeteer captura con CDP.Page.screencast a la misma
    // frecuencia, así no perdemos frames.
    let frame = 0
    ...
    function tick(now) {
      // Determinar el frame ideal según el reloj real para no acumular drift.
      const elapsed = (now - start) / 1000
      const expectedFrame = Math.floor(elapsed * FPS)
      while (frame < expectedFrame && frame < TOTAL_FRAMES) {
        renderFrame(frame)
        frame++
      }
